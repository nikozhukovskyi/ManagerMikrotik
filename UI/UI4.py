    ...
    def update_system_status(self):
        ...
        try:
            ...
            with ConnectHandler(**ssh_device) as ssh_conn:
                ...
                # Оновлюємо версії в базі даних після перевірки
                installed_version, latest_version, routerboard_firmware = check_versions(self.device, None)
                if installed_version and latest_version and routerboard_firmware:
...
class MainWindow(QMainWindow):
    def __init__(self, conn_str):
        ...
        # Додаємо button_frame і розтягуємо простір донизу, потім додаємо footer_label з відступом
        left_layout.addWidget(button_frame)
        left_layout.addStretch()  # Розтягуємо простір, щоб footer_label опустився вниз
        left_layout.addWidget(footer_label)
        left_layout.addSpacing(10)  # Відступ 10 пікселів знизу
    ...
    # Решта методів MainWindow залишаються без змін
    def parse_version(self, version_str):
        if not version_str:
            return (0, 0, 0)
        version_str = re.sub(r'[^0-9.]', '', version_str)
